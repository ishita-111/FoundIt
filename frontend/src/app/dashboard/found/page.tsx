...
export default function FoundItemsPage() {
  ...
  const fetchItems = useCallback(async (filters: Record<string, string> = {}) => {
    setLoading(true);
    try {
      const data = await getItems({ ...filters, type: 'found', page: String(page) }, token || undefined);
      setItems(data.items);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <h1 className="text-3xl font-bold text-white mb-1">📦 Found Items</h1>
        <p className="text-dark-300">Browse items that have been found and reported</p>
      </motion.div>
      ...
      {loading ? (
        ...
        <div className="glass rounded-2xl p-12 text-center">
          ...
          <p className="text-dark-200 text-lg mb-2">No found items yet</p>
